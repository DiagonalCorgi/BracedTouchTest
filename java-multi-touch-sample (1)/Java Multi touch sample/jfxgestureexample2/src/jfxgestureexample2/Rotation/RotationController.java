...
public class RotationController extends Arc {
    ...
    public void onTouchPressed(TouchEvent t) {
     ...
     public void handle(ActionEvent E) {
        ...
        InteractionLogger.log(Level.INFO, "Angle of target is " + target.getRotate());
    }
    ...
    public void setOnRotate(RotateEvent E) {
        source.setOnRotate(new EventHandler<RotateEvent>() {
            @Override public void handle(RotateEvent event) {
                ...
                InteractionLogger.log(Level.INFO, "Rectangle: Rotate Angle " + (source_angle));
                double angle_checker = source_angle - target.getRotate();
                if (angle_checker < 1 && angle_checker >-1) {
                    InteractionLogger.log(Level.INFO, "SUCCESS");
                    source.setStyle("-fx-fill: blueviolet");
                    target.setStyle("-fx-fill: red");
                }
                else {
                    source.setStyle("-fx-fill: cyan");
                    target.setStyle("-fx-fill: cyan");
                }
